import { Camera, PieChart, Target, Lightbulb, TrendingUp, Globe } from "lucide-react";
...
function FeatureCard({ icon: Icon, title, description, index }) {
  return (
    <div
      ...
      className="bg-white rounded-2xl p-6 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)]
                 transition-all duration-300 hover:-translate-y-1 group"
    >
      <div className="w-12 h-12 rounded-full bg-[#FFF0E6] flex items-center justify-center mb-4
                      group-hover:bg-[#FF7A45] group-hover:text-white transition-colors duration-300">
        <Icon className="w-5 h-5 text-[#FF7A45] group-hover:text-white transition-colors duration-300" />
      </div>
